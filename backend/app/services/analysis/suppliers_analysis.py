...
def ranking_fornecedores(df: pd.DataFrame, top: int = 10) -> pd.DataFrame:
    """
    Retorna ranking de fornecedores por avaliação.
    
    Args:
        df: DataFrame com dados de fornecedores
        top: Número de fornecedores a retornar
...
def analyze_suppliers_kpis(df: pd.DataFrame) -> dict:
    """
    Analisa KPIs de fornecedores.
    
    Args:
        df: DataFrame com dados de fornecedores
        
    Returns:
        Dicionário com KPIs de fornecedores
    """
...
def get_product_types_distribution(df: pd.DataFrame) -> pd.Series:
    """
    Retorna distribuição de tipos de produtos fornecidos.
    
    Args:
        df: DataFrame com dados de fornecedores
        
    Returns:
        Series com contagem de tipos de produtos
    """
...
def create_supplier_product_matrix(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Returns:
        DataFrame com matriz fornecedores x produtos
    """
...
def get_top_bottom_suppliers(df: pd.DataFrame, n: int = 5) -> dict:
    ...
    Args:
        df: DataFrame com dados de fornecedores
        n: Número de fornecedores a retornar em cada categoria
        
    Returns:
        Dicionário com 'top' e 'bottom' DataFrames
    """
    if df is None or df.empty or 'Avaliacao' not in df.columns:
...
def get_suppliers_by_state(df: pd.DataFrame) -> pd.Series:
    ...
    Returns:
        Series com contagem por estado
    """
